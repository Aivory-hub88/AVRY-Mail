...
pub async fn list(
    ...
    let per_page: i64 = crate::api::query_i64(params.get("per_page"))
        .unwrap_or(200)
        .min(1000);
    let page: i64 = crate::api::query_i64(params.get("page"))
        .unwrap_or(1)
        .max(1);
    ...
    let rows: Vec<Value> = match &state.db {
        DbPool::Postgres(pool) => {
            ...
            let r = if let Some(mid) = mailbox_id.as_deref() {
...
pub async fn reply(
    ...
    authz::require_thread_access(&state, &headers, thread_id).await?;
    ...
    let mid = handle
        ...
        .map_err(|_| StatusCode::INTERNAL_SERVER_ERROR)?;
    Ok(Json(
        serde_json::json!({"success": true, "data": {"id": mid.to_string()}}),
    ))
}
...
pub async fn crawl(
    ...
    let uid = Uuid::parse_str(&id).map_err(|_| StatusCode::BAD_REQUEST)?;
    authz::require_thread_access(&state, &headers, uid).await?;
    let thread = get_one(State(state.clone()), headers.clone(), Path(id.clone())).await?;
    let data = thread
        .0
        .get("data")
        .cloned()
    ...
    let last = messages
        ...
        .unwrap_or("");
    let last_folder = messages
        .last()
        .and_then(|m| m.get("folder").and_then(|v| v.as_str()))
        .unwrap_or("Inbox");
    // follow-up heuristic: if last is Sent and >2 days old and count <6, needs follow-up
    let days_since = if let Ok(dt) = chrono::DateTime::parse_from_rfc3339(last) {
    ...
    let suggested = if needs_follow_up {
        ...
        let subj2 = if subj.to_lowercase().starts_with("re:") {
            subj.to_string()
        } else {
            format!("Re: {}", subj)
        };
        ...
        let reason = format!(
            "Last sent {} days ago, no reply in thread ({} messages)",
            days_since, count
        );
...
pub async fn follow_up(
    ...
    let crawled = crawl(State(state.clone()), headers, Path(id.clone())).await?;
    let suggested = crawled
        .0
        .get("data")
        .and_then(|d| d.get("crawl"))
        .and_then(|c| c.get("suggested_follow_up"))
        .cloned()
        .unwrap_or(serde_json::Value::Null);
    if suggested.is_null() {
        return Ok(Json(
            serde_json::json!({"success": true, "data": {"needed": false, "reason": "No follow-up needed at this time"}}),
        ));
    }
    Ok(Json(
        serde_json::json!({"success": true, "data": {"needed": true, "draft": suggested}}),
    ))
}
