use aivory_mail_core::{
    intelligence,
    parser::{parse_raw_email, snippet_from_body},
    types::SendRequest,
};
...
pub async fn handle_inbound_raw_with_folder(
    ...
    info!(
        "inbound parsed from={} to={} subject={:?} size={} forced_folder={:?}",
        from,
        to,
        parsed.subject,
        raw.len(),
    ...
    let probe = match forced_folder.as_deref() {
        Some(f)
            if f.eq_ignore_ascii_case("Sent")
                || f.eq_ignore_ascii_case("Drafts")
                || f.eq_ignore_ascii_case("Draft") =>
        {
            from
        }
        _ => to,
    ...
    crate::api::contacts::upsert_from_address(&state.db, &mailbox_id, &from_email, &from_name)
        .await;
    let mut folder = if let Some(ff) = forced_folder.clone() {
        // For import/migration, respect original folder (Inbox/Sent/Drafts etc)
        ff
    } else if is_blocked(&state.db, &mailbox_id, &from_email).await {
        "Spam".to_string()
    } else {
        "Inbox".to_string()
    };
    ...
    if forced_folder.is_none() {
        match apply_filters(&state.db, &mailbox_id, &from_email, &subject, &body_for_ai).await {
            ...
            Some(aivory_mail_core::filters::FilterAction::Block) => {
                ...
                let _ = crate::api::contacts::upsert_from_address(
                    &state.db,
                    &mailbox_id,
                    &from_email,
                    &from_name,
            ...
            Some(aivory_mail_core::filters::FilterAction::Forward(addr)) => {
                ...
                let fwd_req = SendRequest {
                    from: to.to_string(),
                    to: vec![addr.clone()],
                    cc: None,
                    bcc: None,
                    subject: subject.clone(),
                    text: Some(body_for_ai.clone()),
                    html: None,
                    attachments: None,
                    thread_id: None,
                    in_reply_to: None,
                ...
                tokio::spawn(async move {
                    let _ = crate::mail::outbound::send_email(&state_fw, fwd_req).await;
                });
            ...
            Some(aivory_mail_core::filters::FilterAction::Move(f)) => {
                folder = f;
            }
            _ => {
                // fallback to core filters resolver (origin/main logic) if no direct match
                let resolved = resolve_filtered_folder(
                    state,
                    &mailbox_id,
                    &from_email,
                    &subject,
    ...
    let msg_uid = parsed
        .message_id
        .clone()
        .unwrap_or_else(|| format!("<{}@aivory.local>", msg_id));
    ...
    if !cid_att_ids.is_empty() {
        if let Some(html) = &mut parsed.body_html {
            for (cid, att_id) in &cid_att_ids {
                let url = format!(
                    "{}/v1/messages/{}/attachments/{}",
                    state.config.public_base_url, msg_id, att_id
                );
                *html = html
                    .replace(&format!("cid:{}", cid), &url)
                    .replace(&format!("cid:<{}>", cid), &url);
            }
        }
    }
    ...
    for att in &parsed.attachments {
        ...
        state
            .store
            .put(&key, att.data.clone(), &att.content_type)
            .await?;
        insert_attachment(
            state,
            &att_id,
    ...
    // 6b. Cognee graph_remember (sidecar t_<user>.mail_ops) — shape B bulk ingest, non-blocking
    {
        ...
        tokio::spawn(async move {
            ...
            if let Err(e) = crate::mail::cognee_client::remember_email(
                &tenant_cog,
                &agent_type,
                &subj_cog,
                &body_cog,
    ...
    tokio::spawn(async move {
        if let Err(e) = trigger_intelligence_hooks(
            &state_clone,
            &msg_id,
            &subject_clone,
            &body_for_ai,
    ...
    // 7b. Webhooks dispatch (Mailflare parity) — async fire to all enabled webhooks for email.received
    {
        ...
        tokio::spawn(async move {
            crate::api::webhooks_registry::trigger_for_event(
                &state_wh,
                "email.received",
                payload_wh,
            )
    ...
    // 7c. Agent tasks auto-create for high signal (Mailflare agent inbox parity)
    {
        ...
        tokio::spawn(async move {
            let state_str = if intel_ag.urgency == aivory_mail_core::types::Urgency::High {
                "needs_reply"
            } else if intel_ag.intent == "invoice" {
                "needs_approval"
            } else {
                "fyi"
            };
            // only create for actionable intents to avoid noise
            if intel_ag.intent == "invoice"
                || intel_ag.intent == "meeting_request"
                || intel_ag.urgency == aivory_mail_core::types::Urgency::High
            {
                let _ = crate::api::agent_tasks::create_internal(
                    state_ag.clone(),
                    serde_json::json!({
    ...
    if let Some(wf_url) = &state.config.workflow_url {
        ...
        tokio::spawn(async move {
            let _ = reqwest::Client::new()
                .post(format!("{}/webhook/email-received", wf_url))
                .json(&payload)
                .send()
                .await;
        });
...
async fn find_or_create_thread(
    ...
    let norm_subject = subject
        .trim()
        .trim_start_matches(|c: char| c == ' ')
        .to_string();
    let norm = regex::Regex::new(r"(?i)^(re|fwd|fw):\s*")
        .unwrap()
        .replace(&norm_subject, "")
        .to_string();
...
async fn is_blocked(db: &aivory_mail_storage::db::DbPool, mailbox_id: &Uuid, email: &str) -> bool {
    if email.is_empty() {
        return false;
    }
    match db {
        aivory_mail_storage::db::DbPool::Postgres(pool) => {
            let r: Option<String> = sqlx::query_scalar("SELECT blocked::text FROM contacts WHERE tenant_id::text='default' AND mailbox_id=$1 AND lower(email)=lower($2) LIMIT 1")
                .bind(mailbox_id.to_string()).bind(email).fetch_optional(pool).await.unwrap_or(None).flatten();
            matches!(r.as_deref(), Some("1") | Some("true") | Some("t"))
        }
        aivory_mail_storage::db::DbPool::Sqlite(pool) => {
            let r: Option<i32> = sqlx::query_scalar("SELECT blocked FROM contacts WHERE tenant_id='default' AND mailbox_id=? AND lower(email)=lower(?) LIMIT 1")
                .bind(mailbox_id.to_string()).bind(email).fetch_optional(pool).await.unwrap_or(None).flatten();
...
async fn apply_filters(
    db: &aivory_mail_storage::db::DbPool,
    mailbox_id: &Uuid,
    from: &str,
    subject: &str,
    body: &str,
) -> Option<aivory_mail_core::filters::FilterAction> {
    use aivory_mail_core::filters::{FilterAction, FilterRule};
    let rows: Vec<(String, String, i32)> = match db {
        aivory_mail_storage::db::DbPool::Postgres(pool) => {
            let rows = sqlx::query("SELECT criteria_json, action_json, COALESCE(priority,0) as priority FROM mail_filters WHERE tenant_id::text='default' AND (mailbox_id=$1 OR mailbox_id IS NULL) AND enabled::text IN ('1','true','t') ORDER BY priority ASC, created_at ASC")
                .bind(mailbox_id.to_string()).fetch_all(pool).await.ok()?;
            rows.into_iter()
                .map(|r| {
                    (
                        r.get::<String, _>("criteria_json"),
                        r.get::<String, _>("action_json"),
        ...
        aivory_mail_storage::db::DbPool::Sqlite(pool) => {
            let rows = sqlx::query("SELECT criteria_json, action_json, COALESCE(priority,0) as priority FROM mail_filters WHERE tenant_id='default' AND (mailbox_id=? OR mailbox_id IS NULL) AND enabled=1 ORDER BY priority ASC, created_at ASC")
                .bind(mailbox_id.to_string()).fetch_all(pool).await.ok()?;
            rows.into_iter()
                .map(|r| {
                    (
    ...
    let parsed: Vec<(serde_json::Value, serde_json::Value, i32)> = rows
        .iter()
        .map(|(c, a, p)| {
            (
        ...
        .collect();
    let rules: Vec<FilterRule> = parsed
        .iter()
        .map(|(c, a, p)| FilterRule {
            criteria: c,
            action: a,
            priority: *p,
        })
...
async fn maybe_send_vacation_reply(
    ...
    let lower_headers: std::collections::HashMap<String, String> = headers
        .iter()
    ...
    if lower_headers.get("x-auto-response-suppress").is_some() {
        return Ok(());
    }
    if lower_headers
        .get("precedence")
    ...
    if let Some(e) = end_at {
        if let Ok(dt) = chrono::DateTime::parse_from_rfc3339(&e) {
            if now > dt.with_timezone(&Utc) {
                return Ok(());
            }
        }
    ...
    if let Some(ls) = last_sent {
        if (now - ls).num_days() < interval_days as i64 {
            return Ok(());
        }
    }
    ...
    let mailbox_addr: String = match &state.db {
        aivory_mail_storage::db::DbPool::Postgres(pool) => {
            sqlx::query_scalar("SELECT address FROM mailboxes WHERE id=$1")
                .bind(mailbox_id)
                .fetch_optional(pool)
                .await?
                .unwrap_or_default()
        ...
        aivory_mail_storage::db::DbPool::Sqlite(pool) => {
            sqlx::query_scalar("SELECT address FROM mailboxes WHERE id=?")
    ...
    if mailbox_addr.is_empty() || mailbox_addr.to_lowercase() == from_email.to_lowercase() {
        return Ok(());
    }
    let vac_subject = if subject.is_empty() {
    ...
    let req = SendRequest {
        from: mailbox_addr.clone(),
        to: vec![from_email.to_string()],
        cc: None,
        bcc: None,
        subject: vac_subject,
        text: Some(vac_body.clone()),
        html: None,
        attachments: None,
        thread_id: None,
        in_reply_to: None,
...
async fn maybe_forward(
    ...
    let Some(fwd) = forward_to else {
        return Ok(());
    };
    let fwd = fwd.trim().to_string();
    if fwd.is_empty() || fwd.eq_ignore_ascii_case(to) || fwd.eq_ignore_ascii_case(from) {
        return Ok(());
    }
...
async fn insert_message(
    ...
    msg_uid: &str,
    parsed: &aivory_mail_core::parser::ParsedEmail,
    snippet: &str,
    raw_key: &str,
...
async fn resolve_filtered_folder(
    state: &Arc<AppState>,
    mailbox_id: &Uuid,
    from: &str,
    subject: &str,
    ...
    let rows: Vec<(String, String, i32)> = match &state.db {
        aivory_mail_storage::db::DbPool::Postgres(pool) => {
            sqlx::query("SELECT criteria_json, action_json, COALESCE(priority,0) as priority FROM mail_filters WHERE tenant_id::text='default' AND (mailbox_id=$1 OR mailbox_id IS NULL) AND enabled=true ORDER BY priority ASC, created_at ASC")
                .bind(mailbox_id.to_string())
        ...
        aivory_mail_storage::db::DbPool::Sqlite(pool) => {
            sqlx::query("SELECT criteria_json, action_json, COALESCE(priority,0) as priority FROM mail_filters WHERE tenant_id='default' AND (mailbox_id=? OR mailbox_id IS NULL) AND enabled=1 ORDER BY priority ASC, created_at ASC")
                .bind(mailbox_id.to_string())
                .fetch_all(pool).await.ok()
                .map(|rs| rs.into_iter().map(|r| (r.get("criteria_json"), r.get("action_json"), r.try_get::<i32,_>("priority").unwrap_or(0))).collect())
                .unwrap_or_default()
    ...
    let parsed: Vec<(serde_json::Value, serde_json::Value, i32)> = rows
        ...
        .collect();
    let rules: Vec<aivory_mail_core::filters::FilterRule> = parsed
        .iter()
        .map(|(c, a, p)| aivory_mail_core::filters::FilterRule {
            criteria: c,
            action: a,
            priority: *p,
        })
        .collect();
    aivory_mail_core::filters::resolve_folder(&rules, from, subject, body)
        .unwrap_or_else(|| "Inbox".to_string())
}

const AUTO_REPLY_SKIP_PREFIXES: &[&str] =
...
#[allow(dead_code)]
async fn maybe_send_vacation_reply_simple(
    state: &Arc<AppState>,
    mailbox_id: &Uuid,
    from: &str,
    subject: &str,
    ...
    if sender.is_empty()
        || AUTO_REPLY_SKIP_PREFIXES
            .iter()
            .any(|p| sender.starts_with(p))
    {
        return Ok(());
    }
    ...
    let Some(mailbox_addr) = mailbox_addr else {
        return Ok(());
    };

    let reply_subject = if vac.subject.trim().is_empty() {
        format!("Re: {}", subject)
    } else {
        vac.subject.clone()
    };
...
async fn trigger_intelligence_hooks(
    state: &Arc<AppState>,
    msg_id: &Uuid,
    subject: &str,
    body: &str,
    intel: &aivory_mail_core::types::IntelligenceResult,
) -> Result<()> {
    if let Some(ai_url) = &state.config.ai_gateway_url {
        ...
        let _ = reqwest::Client::new()
            .post(format!("{}/v1/mail/intelligence", ai_url))
...
pub async fn import_message(
    ...
    let msg_uid = parsed
        .message_id
        .clone()
        .unwrap_or_else(|| format!("<import-{}@aivory.local>", Uuid::new_v4()));
    ...
    let already_exists = match &state.db {
        ...
        aivory_mail_storage::db::DbPool::Sqlite(pool) => {
            sqlx::query_scalar::<_, i64>(
                "SELECT COUNT(*) FROM messages WHERE mailbox_id=? AND message_id=?",
            )
            .bind(mailbox_id.to_string())
            .bind(&msg_uid)
            .fetch_one(pool)
