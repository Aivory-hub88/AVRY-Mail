...
pub async fn resolve_recipient(state: &Arc<AppState>, to: &str) -> Result<RecipientResolution> {
    ...
    match &state.db {
        aivory_mail_storage::db::DbPool::Postgres(pool) => {
            ...
            // send-as alias: mail addressed to a Compose "Send As" alias
            // (distinct from the mailbox_aliases table above, which backs
            // Admin -> Aliases) was rejected outright because only
            // mailboxes/mailbox_aliases were checked — deliver it into the
            // owning mailbox instead.
            if let Some(row) = sqlx::query(
