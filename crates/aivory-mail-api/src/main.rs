...
async fn ensure_schema(db: &DbPool) -> anyhow::Result<()> {
    let stmts = vec![
        ...
        "CREATE TABLE IF NOT EXISTS mailboxes (id TEXT PRIMARY KEY, tenant_id TEXT NOT NULL, domain_id TEXT NOT NULL, address TEXT UNIQUE NOT NULL, display_name TEXT, is_catch_all INTEGER NOT NULL DEFAULT 0, use_all_domains INTEGER NOT NULL DEFAULT 0, forward_to TEXT, password_hash TEXT, password_hash_dovecot TEXT, imap_password_encrypted TEXT, avatar_content_type TEXT, avatar_updated_at TEXT, created_at TEXT NOT NULL)",
        ...
        "CREATE UNIQUE INDEX IF NOT EXISTS uq_send_as_alias_mailbox_email ON send_as_aliases (mailbox_id, lower(alias_email))",
    ...
    let alters = vec![
        ...
        "ALTER TABLE mailboxes ADD COLUMN avatar_content_type TEXT",
        "ALTER TABLE mailboxes ADD COLUMN avatar_updated_at TEXT",
    ];
    for sql in stmts {
        match db {
