    ...
    <div className="min-h-screen bg-[#f8f6ef] dark:bg-zinc-900">
      <div className="mx-auto max-w-4xl p-6">
        ...
        <div className="flex items-center justify-between">
          <div className="text-sm text-zinc-500 dark:text-zinc-400">Pages / <span className="font-semibold text-[#202124] dark:text-white">Overview</span></div>
          <div className="flex items-center gap-2">
            <span className="flex items-center gap-2 rounded-lg border border-[#e8e0c8] bg-[#fefcf6] px-3 py-1.5 text-sm dark:border-zinc-700 dark:bg-zinc-800 dark:text-zinc-200"><span className="h-2 w-2 rounded-full bg-emerald-500" /> Operational</span>
            <span className="hidden sm:inline text-zinc-400">⋯</span>
          </div>
        </div>
        <h1 className="mt-2 text-3xl font-bold">Overview</h1>
        <div className="mt-3 flex gap-2">
          <a href="/domains" target="_top" className="rounded-lg border border-zinc-200 bg-white px-3 py-1.5 text-xs font-medium hover:bg-zinc-50 dark:border-zinc-700 dark:bg-zinc-800 dark:text-zinc-200 dark:hover:bg-zinc-700">Domains</a>
          <a href="/settings/mail" target="_top" className="rounded-lg border border-zinc-200 bg-white px-3 py-1.5 text-xs font-medium hover:bg-zinc-50 dark:border-zinc-700 dark:bg-zinc-800 dark:text-zinc-200 dark:hover:bg-zinc-700">Mail settings</a>
        ...
        {isAdmin && (
        <div className="mt-6 rounded-2xl border border-[#e8e0c8] bg-[#fefcf6] p-4 shadow-sm dark:border-zinc-700 dark:bg-zinc-800">
          <div className="flex flex-wrap items-center gap-3">
            <span className="text-sm text-zinc-500 dark:text-zinc-400">default</span>
            <span className="rounded bg-zinc-100 px-2 py-0.5 text-xs dark:bg-white/10 dark:text-zinc-300">dev</span>
            <span className="text-sm text-zinc-400 dark:text-zinc-500">1</span>
            <div className="ml-auto flex flex-1 items-center gap-2 sm:ml-4">
              <div className="flex-1 rounded-lg border border-[#e8e0c8] bg-[#f8f6ef] px-3 py-1.5 font-mono text-sm dark:border-zinc-700 dark:bg-zinc-900 dark:text-zinc-100">
                {showRaw ? showRaw : (keys[0]?.key_masked || "avry-dev-************************")}
              </div>
              <button onClick={()=> setShowRaw(showRaw ? null : (keys[0] ? rawMap[keys[0].id] || keys[0].key_masked : null))} className="rounded p-1.5 text-zinc-500 hover:bg-zinc-100 dark:text-zinc-400 dark:hover:bg-white/10" title="Reveal">
              ...
              <button onClick={()=> navigator.clipboard?.writeText(showRaw || rawMap[keys[0]?.id] || "")} className="rounded p-1.5 text-zinc-500 hover:bg-zinc-100 dark:text-zinc-400 dark:hover:bg-white/10" title="Copy">
              ...
              <button onClick={create} className="rounded p-1.5 text-zinc-500 hover:bg-zinc-100 dark:text-zinc-400 dark:hover:bg-white/10" title="Create">
              ...
              <button onClick={()=> keys[0] && del(keys[0].id)} className="rounded p-1.5 text-zinc-500 hover:bg-zinc-100 dark:text-zinc-400 dark:hover:bg-white/10" title="Delete">
        ...
        {/* Coupon */}
        <div className="mt-4 rounded-2xl border border-[#f0ece0] bg-[#fefcf6] p-5 shadow-sm dark:border-zinc-700 dark:bg-zinc-800">
        ...
        <div className="mt-4 rounded-2xl border border-[#f0ece0] bg-[#fefcf6] p-5 shadow-sm dark:border-zinc-700 dark:bg-zinc-800">
          ...
          {mcpLink && (
            <div className="mt-3 flex gap-2">
              <input readOnly value={mcpLink} className="flex-1 rounded-lg border border-[#e8e0c8] bg-[#f8f6ef] px-3 py-2 font-mono text-xs dark:border-zinc-700 dark:bg-zinc-900 dark:text-zinc-100" />
              <button onClick={()=> navigator.clipboard?.writeText(mcpLink)} className="rounded-lg border border-[#e8e0c8] bg-[#fefcf6] px-3 py-2 text-xs hover:bg-[#f8f6ef] dark:border-zinc-700 dark:bg-zinc-800 dark:hover:bg-zinc-700">Copy</button>
            </div>
          )}
          <div className="mt-2 text-xs text-zinc-400 dark:text-zinc-500">MCP: POST https://mail.aivory.uk/mcp with Authorization: Bearer &lt;api_key&gt; or ?api_key=</div>
        </div>
        )}
          ...
          {myToken && (
            <div className="mt-3">
              <div className="flex gap-2">
                <input readOnly value={myToken} className="flex-1 rounded-lg border border-[#e8e0c8] bg-[#f8f6ef] px-3 py-2 font-mono text-xs dark:border-zinc-700 dark:bg-zinc-900 dark:text-zinc-100" />
                <button onClick={()=> navigator.clipboard?.writeText(myToken)} className="rounded-lg border border-[#e8e0c8] bg-[#fefcf6] px-3 py-2 text-xs hover:bg-[#f8f6ef] dark:border-zinc-700 dark:bg-zinc-800 dark:hover:bg-zinc-700">Copy</button>
              </div>
              <div className="mt-1 text-xs text-amber-700 dark:text-amber-400">This token is shown once — copy it now. You can revoke it below any time.</div>
            </div>
          )}
          <div className="mt-2 text-xs text-zinc-400 dark:text-zinc-500">MCP: POST https://mail.aivory.uk/mcp with Authorization: Bearer &lt;token&gt;</div>

          {myGrants.length > 0 && (
            <div className="mt-4 border-t border-[#f0ece0] pt-3 dark:border-zinc-700">
              <div className="text-xs font-semibold text-zinc-500 dark:text-zinc-400">Your connected clients</div>
              <div className="mt-2 space-y-2">
                {myGrants.map((g:any)=> (
                  <div key={g.id} className="flex items-center justify-between rounded-lg border border-[#e8e0c8] bg-[#f8f6ef] px-3 py-2 text-xs dark:border-zinc-700 dark:bg-zinc-900">
                    <div>
                    ...
                    {!g.revoked_at && (
                      <button onClick={()=> revokeMyGrant(g.id)} className="rounded border border-[#e8e0c8] px-2 py-1 hover:bg-white dark:border-zinc-600 dark:hover:bg-white/10">Revoke</button>
                    )}
                  </div>
                ))}
              </div>
