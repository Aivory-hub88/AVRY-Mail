...
## 8. Current production status (as of 2026-09-13, updated same day)
...
- Migrations 018–029 are fully applied (`_sqlx_migrations` ledger verified, checked both before and after the change below).
- **`AVRY_MCP_CAPABILITY_MODE=v2` is now permanently enabled in production `.env`** (set 2026-09-13, commit `cb9abc7` wired the env passthrough in `docker-compose.mail-prod.yml`). The legacy `/mcp` handler remains live and reachable alongside it, unchanged, per §2.
- A full 9-step controlled canary (documented in `STAGING_MCP_E2E.md`) was first executed against production on 2026-09-12 and reverted (v2 left dormant afterward) — that run validated the isolation model on the pre-Phase-2-permanent binary.
- The **permanent enablement** was done and verified later on 2026-09-13, following the same discipline: migration ledger preflight (clean before and after) → fresh `pg_dump` backup + SHA-256 checksum → `.env` updated → `avry-mail` service recreated → confirmed `healthy` → real capability grant issued for the admin's own mailbox (`mail.read`, `mail.search`, 600s) → `tools/list` returned the full v2 catalog → `get_inbox_overview` returned real data (`total:178, unread_inbox:0`) → an invalid bearer token got `401` → the grant was revoked → the same token immediately got `401` again, confirming revocation is instant → grant cleaned up. No revert step follows this one; it's intentionally permanent.
- One operational note from this rollout: the production hardening overlay (`docker-compose.mail-prod.hardening.yml`) sets `ports: !reset []` on `avry-mail` — the API has no host-published port by design (Traefik is the sole ingress at `mail.aivory.uk`). Verification calls during the canary were therefore run via `docker exec avry-mail curl http://127.0.0.1:8095/...` from inside the container's own network namespace, not from the host. Anyone repeating this canary should do the same rather than trying to reach `127.0.0.1:8095` from the VPS host directly.
- A live isolated staging stack (`avry-mail-staging` + `avry-postgres-staging` on the same VPS, `AVRY_MCP_CAPABILITY_MODE=v2` permanently on) still exists for regression-testing future v2 changes without touching production.
...
- **Prompt hardening at the assistant/orchestration layer (Cerveau) is not implemented.** §5's sanitizer closes the invisible-character smuggling vector only; visible-instruction injection defense is still open. This is now the most important remaining gap given v2 is live: any agent consuming v2 tool results (e.g. Cerveau via MCP) gets raw email content with no injection scanning or untrusted-content framing on its side. A concrete closure plan exists in `AVRY-Cerveau/docs/CERVEAU-MCP-TOOL-RESULT-PROMPT-HARDENING-PLAN.md` (written 2026-09-13) — it hasn't been implemented yet.
- `AVRY_MCP_CAPABILITY_MODE=v2` is now permanently enabled (see updated §8) — the items above (incomplete `ExecutionContext` coverage, no attachment tools, no assistant-layer hardening) did **not** block that decision and remain open regardless of the flag. Treat them as live risk, not pre-conditions that were satisfied.
