...
### UI direction — Aivory Mail visual system (in progress / next)

The next design pass keeps the existing inbox, compose, calendar, sharing surfaces, and motion work while refining the visual system toward a light, spacious, blue-accented interface. It preserves Manrope as the brand typeface and the existing reduced-motion safeguards.

- **Palette:** warm light surfaces, white cards, soft borders, and restrained blue accents
- **Typography:** Manrope across the product, with clear hierarchy and compact utility labels
- **Layout:** spacious rounded panels, dense-but-readable message rows, and responsive folder navigation
- **Motion:** dialog scale, grid transitions, and message navigation progress with `prefers-reduced-motion` support

Aivory's current inbox remains functional during this work. The redesign is a separate follow-up with no behavior changes in the visual pass.
